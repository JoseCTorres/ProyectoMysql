...
class Program
{
    public static string connectionString = "Server=localhost;Database=Papeleria2;Uid=root;Pwd=Programacion;";
    ...
    static string ConvertirABase64(byte[] bytes)
    {
    ...
    static bool InicioSesion()
    {
        ...
        string contraseñaHexadecimal = ConvertirABase64(contraseñaCifrada);
        ...
        using (MySqlConnection conn = new MySqlConnection(connectionString))
        {
            try
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand(query, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@Contraseña", contraseñaHexadecimal);  

                    var result = cmd.ExecuteScalar();

                    if (result != null)
                    {
                        ID_UsuarioLogueado = Convert.ToInt32(result);
            ...
            try
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand(query, conn))
                {
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            string estatus = reader["Estatus"].ToString() == "1" ? "Activo" : "Inactivo";
